...
CART_EXPIRE_HOURS = int(os.environ.get('CART_EXPIRE_HOURS', '1'))
...
class CartManager:
    ...
    async def save(self, response: Response):
        self.cart['expiration_time'] = datetime.now(timezone.utc) + timedelta(hours=CART_EXPIRE_HOURS)
    ...
    def get_discount(self):
        ...
        if min_purchase > 0 and total < min_purchase:
            return Decimal(0)
        if coupon.get("discount_type") == "fixed":
            return min(Decimal(coupon["discount"]), total)
        return (Decimal(coupon["discount"]) / Decimal(100)) * total
